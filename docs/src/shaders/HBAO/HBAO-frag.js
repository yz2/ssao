...
#define NUM_SAMPLE_DIRS 12.0
#define NUM_SAMPLE_STEPS 10.0
...
vec3 random (vec2 st) {
    float term1 = sin(dot(st.xy, vec2(12.9898,78.233)))*437635.769;
    float term2 = sin(dot(st.xy, vec2(12.9898,78.233)))*575593.023850;
    float term3 = sin(dot(st.xy, vec2(12.9898,78.233)))*778464.36792;
    return normalize(vec3((term1 - floor(term1)) * 2.0 - 1.0, (term2 - floor(term2)) * 2.0 - 1.0, (term3 - floor(term3))) * 2.0 - 1.0);
}
...
void main() {
    ...
    for (float i = 0.0; i < NUM_SAMPLE_DIRS; i += 1.0) {
        ...
        for (float j = 1.0; j < NUM_SAMPLE_STEPS + 1.0; j += 1.0) {
            ...
            vec2 uv = offset.xy * 2.0 - vec2(1.0);
            vec4 viewS = inverseProjectionMatrix * vec4(uv, 0.5, 1.0);
            vec3 horizonPosition = viewS.xyz * sampleDepth/viewS.z;
