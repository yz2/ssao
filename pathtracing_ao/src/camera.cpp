...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // Vector3D( tan(radians(hFov)*.5),  tan(radians(vFov)*.5),-1)
  // So width is 2*tan(radians(hFov)*.5),
  //    height is 2*tan(radians(vFov)*.5)

  Vector3D bottom_left = Vector3D(-tan(radians(hFov)*.5), -tan(radians(vFov)*.5),-1);
  Vector3D offset = Vector3D(x*2*tan(radians(hFov)*.5), y*2*tan(radians(vFov)*.5), 0);
  Vector3D direction = c2w*(bottom_left + offset);
  direction.normalize();
