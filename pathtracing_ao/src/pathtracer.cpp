...
void PathTracer::key_press(int key) {
  ...
  Spectrum PathTracer::estimate_ambient_occlusion(const Ray& r, const Intersection& isect) {
  // Estimate ambient occlusion from this intersection.
  ...
  uint_fast16_t num_samples = scene->lights.size() * ns_area_light;
  Spectrum L_out;
  uint_fast16_t hits = 0;
  Intersection sample_isect; // Will use this same intersection object for every sample
  for (uint_fast16_t i = 0; i < num_samples; i++) {
    Vector3D w_in = hemisphereSampler->get_sample();
    Vector3D w_in_world = o2w*w_in;
    Vector3D offset = EPS_D*w_in_world;
    Ray sample_ray(hit_p + offset, w_in_world);
    if (bvh->intersect(sample_ray, &sample_isect) and sample_ray.max_t < max_t) {
      hits += 1;
    }
  }

  float proportion = (float) (num_samples - hits) / (float) num_samples;
  return Spectrum(proportion, proportion, proportion);
}
  ...
  auto get_sample = [&](Vector2D sample_coord) -> Spectrum {
      Intersection isect;
      Ray new_ray = camera->generate_ray(sample_coord.x / sampleBuffer.w,
                                         sample_coord.y / sampleBuffer.h);
      new_ray.depth = max_ray_depth;
      if (!bvh->intersect(new_ray, &isect))
        return Spectrum(0, 0, 0);
      return estimate_ambient_occlusion(new_ray, isect);
  };

  // Add a sample to the average and adaptive sampling vars
  Spectrum spectrum_avg(0, 0, 0);
  auto include_sample = [&]() {
    Vector2D offset;
    if (ns_aa == 1) { offset = Vector2D(0.5, 0.5); }
    else { offset = gridSampler->get_sample();}
    spectrum_avg += get_sample(origin + offset);;
  };
